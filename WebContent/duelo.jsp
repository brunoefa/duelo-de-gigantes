<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<jsp:include page="cabecalho.jsp" />

	<div class="jumbotron">
		<h1>Hora de Duelar</h1>
		<p class="lead">Escolha seu guerreiro e comece a batalha. Torça
			para que seu exercito seja mais forte do que o rival.</p>
		<p>
			<a class="btn btn-lg btn-success" href="#" role="button">Quero
				cadastrar um novo Duelo!</a>
		</p>
	</div>

	<div class="row marketing">
		<div class="col-lg-6">
			<div class="center">
				<a href="#"> <img data-src="holder.js/300x300"
					class="img-circle" alt="Oponente 1"
					src="data:image/svg+xml;base64,"
					data-holder-rendered="true" style="width: 300px; height: 300px;">
				</a>
			</div>
			<h2 class="center">Oponente 1</h2>
		</div>

		<div class="col-lg-6">
			<div class="center">
				<a href="#"> <img data-src="holder.js/300x300"
					class="img-circle" alt="300x300"
					src="data:image/svg+xml;base64,"
					data-holder-rendered="true" style="width: 300px; height: 300px;">
				</a>
			</div>
			<h2 class="center">Oponente 2</h2>
		</div>
		<p class="center action-bar">
			<a class="btn btn-lg btn-info" href="#" role="button">Próximo
				Duelo</a>
		</p>

	</div>

</div>

<jsp:include page="rodape.jsp" />